...
begin
    ...
    using CVFEMSystems: finitebell, d1finitebell, d2finitebell, ∇ηΛ∇, hminmax, ΛMatrix, ScalarTestData, paraprod,hdirichlet, hneumann
    using CVFEMSystems: coord, transmission, nnodes, nedges, volume, edgenode, dirichlet!, minplot, fourplots, runconvergence
...
function celleval(y, u, celldata, userdata)
        ...
        femstiffness!(S, celldata.shapegradients, Λ)
        ω = volume(celldata)
		M=local_massmatrix[d]
	    M=Diagonal(fill(1/celldim,celldim))
	    for il = 1:celldim
	        for jl = 1:celldim
				y[il] += S[il,jl]*u[jl]*ω
	            y[il] += M[il,jl]*((u[jl] - uold[jl])/tstep - f(coord(celldata, jl))) * ω
        	end
		end
    end

# ╔═╡ e6400a50-ae58-4e5c-ac32-32f4b00903d5
data=ScalarTestData(Λ=ΛMatrix(100,0*π/4), u=paraprod)

# ╔═╡ 69f8af2f-af08-41c0-beae-b60b9737aa92
grid=randgrid(2,1000; X=(-1,1))
