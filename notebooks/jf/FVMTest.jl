...
# ╔═╡ 68da8158-01cd-4f16-829d-5793300e2bd8
md"""
# Finite volume tests
"""
...
sol1d = fvmsolve(grid1d, [1.0;;], f1d, β)
...
function runconvergence(Λ, ref, gengrid)
    ...
    for n in ref
        ...
        sol = fvmsolve(grid, Λ, X -> -∇Λ∇(finitebell, X, Λ), X -> 0.0)
...
sol2d = fvmsolve(grid2d, Λ2d, f2d, β)
...
sol3d = fvmsolve(grid3d, Λ3d, f3d, β)
...
sol2d_a = fvmsolve(grid2d_a, Λ2d_a, X -> -∇Λ∇(finitebell, X, Λ2d_a), β)
...
sol3d_a = fvmsolve(grid3d_a, Λ3d_a, X -> -∇Λ∇(finitebell, X, Λ3d_a), β)
