...
function celleval(y, u, celldata, userdata)
    ...
    for il = 1:nnodes(celldata)
		for ispec=1:nspec
        y[ispec,il] = ((u[ispec,il] - uold[ispec,il])/tstep - f(coord(celldata, il))) * ω
		end
    end
	...
	for ispec=1:nspec
    	for ie = 1:nedges(celldata)
        	 ...
        	 g =  ΛKL[ie] * (u[ispec,i1] - u[ispec,i2])
        	y[ispec,i1] += g
        	y[ispec,i2] -= g
	    end
	end
...
sys=CVFEMSystem(grid,celleval,hdirichlet,data,2)
...
tsys=CVFEMSystem(tgrid,celleval,hneumann,tdata,2)

# ╔═╡ 05d59d98-64fd-4f3f-b16b-f4765c031684
begin
	u=map(finitebell,tgrid)
	inival=zeros(tsys)
	inival[1,:].=u
	inival[2,:].=u
	inival
...
tsol=solve(tsys; inival, times=0:0.001:0.1)
...
# ╔═╡ 1a262ac0-0858-4f50-9bde-551b8458025c
fourplots(tgrid,tsol,ispec=2)
