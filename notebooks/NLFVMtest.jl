...
function nlfvmtest(grid; Λ=Diagonal(ones(dim_space(grid))),tol=1.0e-10)
...
function runconvergence(ref,dim,gengrid;Λ=Diagonal(ones(dim)))
	h1norms=[]
	l2norms=[]
	h=[]
	for n in ref
...
runconvergence([10*2^k for k=1:10],1,rectgrid)
...
runconvergence([10*4^k for k=1:7], 2,rectgrid)

# ╔═╡ 2bc3ce81-cccd-400a-959a-d8980aff5de9
runconvergence([10*4^k for k=1:7], 2, randgrid)
